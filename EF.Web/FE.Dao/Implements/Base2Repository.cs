...
namespace FE.Dao
{
    ...
    public abstract class Base2Repository<T> : IBase2Repository<T> where T : class
    {
        #region 固定公用帮助，含事务
        ...
        /// </summary>
        public virtual object ExecuteSqlQuery<S>(string sql, params DbParameter[] para)
        {
            try
            {
                return this.Context.Database.SqlQuery<S>(sql, para);
            }
            catch (Exception e)
            {
                throw e;
